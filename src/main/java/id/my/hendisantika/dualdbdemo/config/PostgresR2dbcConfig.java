...
public class PostgresR2dbcConfig {
    ...
    private static final Logger log = LoggerFactory.getLogger(PostgresR2dbcConfig.class);

    private final PostgresProperties properties;

    public PostgresR2dbcConfig(PostgresProperties properties) {
    ...
    private class FailoverConnectionFactory implements ConnectionFactory {
        ...
        FailoverConnectionFactory(List<DatabaseHost> hosts, PostgresProperties props) {
            this.hosts = hosts;
            this.props = props;
            ...
            return Mono.defer(() -> currentPool.create())
                    .retryWhen(Retry.fixedDelay(props.getFailover().getMaxRetries(),
                            ...
                            .doBeforeRetry(signal -> {
                                log.warn("PostgreSQL connection failed, attempting failover. Retry: {}", signal.totalRetries() + 1);
                                switchToNextHost();
                            }))
                    .doOnError(e -> log.error("PostgreSQL connection failed after all retries", e));
        }
